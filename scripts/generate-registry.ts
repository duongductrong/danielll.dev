...
const OUTPUT_FILE = path.join(process.cwd(), 'src/features/craft/registry.generated.ts');
...
const toCamelCase = (filename: string): string => {
  return filename
    .replace(/[-_]/g, ' ')
...
const generateRegistry = (): void => {
  ...
  const blockFiles: BlockInfo[] = fs.readdirSync(BLOCKS_DIR)
  ...
  console.log(`📦 Found ${blockFiles.length} blocks:`, blockFiles.map(b => b.name).join(', '));

  // Generate dynamic imports
  const imports = blockFiles
    .map(block => `const ${block.pascalCase} = dynamic(() => import("./blocks/${block.name}"));`)
    .join('\n');
  ...
  const sourceCodeEntries = blockFiles.map(block => {
...
${blockFiles.map(block => `  ${block.camelCase}: {
    name: "${block.name}",
