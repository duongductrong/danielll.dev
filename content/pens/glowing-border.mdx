...
And the `w-[200%] h-[200%]` class is used to make the gradient larger than the container.
...
export function GlowingBorderPart1() {
  return (
    ...
    >
      <div className="border-2 border-border bg-[#F5F5F5] dark:bg-[#171717] p-4 rounded-xl grid place-items-center size-16">
        <DatabaseZap className="size-6 text-muted-foreground" />
      </div>
    </div>
  );
